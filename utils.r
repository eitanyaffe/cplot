...
# continous
wlegend2=function(fdir, panel, breaks, labels=breaks, title="general", size=1, height, width, make.file=T, tick.size=0.3, border=NA, lwd=2)
{
    N = length(panel)
    step = 1/N
    coords = (1:N)/N
    ...
    for (orient in c("v", "h")) {
        ...
        if (orient == "h") {
            ...
            text(x=coords.breaks, y=-tick.size, labels=labels, pos=1, cex=1)
        } else {
            ...
            text(y=coords.breaks, x=1+tick.size, labels=labels, pos=4, cex=1)
        }
